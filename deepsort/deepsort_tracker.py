...
from deepsort.utils.nms import non_max_suppression
...
class DeepSort(object):
    def __init__(
        ...
        embedder_wts: Optional[str] = None
            Optional specification of path to embedder's model weights. Will default to looking for weights in `deepsort/embedder/weights`. If deepsort is installed as a package and CLIP models is used as embedder, best to provide path.
        ...
        if embedder is not None:
            if embedder not in EMBEDDER_CHOICES:
                ...
                from deepsort.embedder.embedder import (
